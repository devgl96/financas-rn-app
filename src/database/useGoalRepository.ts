...
export function useGoalRepository() {
  ...
  function deleteById(goalId: number) {
    try {
      const statementTransactions = database.prepareSync(`
        DELETE FROM transactions AS t WHERE t.goal_id = $goalId
      `);

      const statementGoals = database.prepareSync(`
        DELETE FROM goals AS g WHERE g.id = $goalId
      `);

      statementTransactions.executeSync({ $goalId: goalId });
      statementGoals.executeSync({ $goalId: goalId });
    } catch (error) {
      throw error;
    }
  }
